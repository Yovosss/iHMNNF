...
from keras.layers import Dense, Input, Dropout, Activation, BatchNormalization, LSTM, Bidirectional
...
from Data_helper import LoadDataImputed
...
arg_parser.add_argument('--label', default=0, type=int, choices=[-1,0,1,2,3,4],
                         help='the label type')

## model
arg_parser.add_argument('--model_type', default='LSTM', choices=['LR', 'RF', 'SVM', 'DNN', 'DNN_EE', 'GRUD', 'HMM_EF', 'HMM_LF', 'LSTM'])
...
model_path = os.path.join(ARGS.working_path, 'model_tuning', 'phase_viii', '48hours', '20230610', 'LSTM_7dim_48hrs_mean2_Paper(optuna)')
if not os.path.exists(model_path):
    os.makedirs(model_path)
output_path = os.path.join(ARGS.working_path, 'output', 'phase_viii', '48hours', '20230610', 'LSTM_7dim_48hrs_mean2_Paper(optuna)')
...
LABEL_DICT = {'-1':'flatten multi-classification', 
              ...
              '4':'HM and SM'}
# Load the data
dataset = LoadDataImputed(data_path=os.path.join('.', 'data', 'phase_viii', '48hours', 'processed', 'imputed_mean_48_2/data4hc_v20220401'), 
...
def build_model(trial):
    ...
    model.add(
        Bidirectional(
            LSTM(units=trial.suggest_int("units", 16, 128, step=4),
                 return_sequences=False, 
                 activation=trial.suggest_categorical("activation", ['relu', 'tanh', 'sigmoid']),
                 recurrent_activation=trial.suggest_categorical("recurrent_activation", ['relu', 'tanh', 'sigmoid']),
                 dropout=trial.suggest_float("dropout", 0.0, 0.5, step=0.05),
                 recurrent_dropout=trial.suggest_float("recurrent_dropout", 0.0, 0.5, step=0.05)
                 ),
                 input_shape=shapes_train
        )
    )
    dropout_choice = trial.suggest_categorical("dropout_lstm", [True, False])
    if dropout_choice == True:
        model.add(Dropout(rate=trial.suggest_float("dropout_lstm_value", 0.0, 0.5, step=0.05)))

    n_layers = trial.suggest_categorical("n_layers", [True, False])
    if n_layers == True:
        model.add(Dense(units=trial.suggest_int("units_fc", 24, 256, step=4),
                             activation=trial.suggest_categorical("activation_fc", ['relu', 'tanh', 'sigmoid']),
        ...
        if dropout_choice == True:
            model.add(Dropout(rate=trial.suggest_float("dropout_fc_value", 0.0, 0.7, step=0.05)))
...
class Objective():
    ...
    def callback(self, study, trial):
        if study.best_trial.number == trial.number:
            self._model.save(os.path.join(model_path, 'LSTM({0})_trial#{1}.h5'.format(LABEL_DICT[str(ARGS.label)], trial.number))) 
# %%
if __name__ == '__main__':
    ...
    study = optuna.create_study(direction='maximize', 
                                ...
                                pruner=optuna.pruners.MedianPruner(),
                                load_if_exists=True,
                                study_name='LSTM(infectious and non-infectious)_20230612215409')
