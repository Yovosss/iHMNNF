...
## model
arg_parser.add_argument('--model_type', default='LR', choices=['LR', 'RF', 'SVM', 'DNN', 'DNN_EE', 'GRUD', 'HMM_EF', 'HMM_LF'])
...
output_path = os.path.join(ARGS.working_path, 'output', 'phase_viii', '48hours', '20220604', 'LR_423dim_48hrs_Paper(optuna)')
...
# %%
print("[Info]: The LR hyperparameter tuning of label {0} is processing".format(LABEL_DICT[str(ARGS.label)]))

def objective(trial):
    # ‘newton-cg’, ‘lbfgs’, ‘sag’ and ‘saga’ handle L2 or no penalty
    # ‘liblinear’ and ‘saga’ also handle L1 penalty
    if ARGS.label == -1 or ARGS.label == 1:
        penalty = trial.suggest_categorical("penalty", ["l1","l2","elasticnet"])
        if penalty == 'l1':
            solver = trial.suggest_categorical("solver_l1", ['liblinear', 'saga'])
            model = LogisticRegression(C=trial.suggest_categorical("C", [1.e-03, 1.e-02, 1.e-01, 1.e+00, 1.e+01, 1.e+02, 1.e+03]),
                                       penalty=penalty,
                                       solver=solver,
                                       multi_class='ovr', 
                                       max_iter=100,
            ...
            solver = trial.suggest_categorical("solver_l2", ['newton-cg', 'lbfgs', 'sag', 'liblinear', 'saga'])
            model = LogisticRegression(C=trial.suggest_categorical("C", [1.e-03, 1.e-02, 1.e-01, 1.e+00, 1.e+01, 1.e+02, 1.e+03]),
                                       penalty=penalty,
                                       solver=solver,
                                       multi_class='ovr', 
            ...
            solver = trial.suggest_categorical("solver_elasticnet", ['saga'])
            model = LogisticRegression(C=trial.suggest_categorical("C", [1.e-03, 1.e-02, 1.e-01, 1.e+00, 1.e+01, 1.e+02, 1.e+03]),
...
if __name__ == '__main__':
    ...
    if ARGS.trainORvalidation == 'Train':
        ...
        if 'solver_elasticnet' in hps:
            C, penalty, solver, l1_ratio = hps['C'], hps['penalty'], hps['solver_elasticnet'], hps['l1_ratio']
        elif 'solver_l1' in hps:
            C, penalty, solver, l1_ratio = hps['C'], hps['penalty'], hps['solver_l1'], None
        else:
            C, penalty, solver, l1_ratio = hps['C'], hps['penalty'], hps['solver_l2'], None

        if ARGS.label == -1:
            model=LogisticRegression(C=C, penalty=penalty, solver=solver, l1_ratio=l1_ratio, multi_class='ovr', verbose=0)
            model.fit(X_train, y_train)

            # save the model
            model_file = os.path.join(model_path, 'LR({}).pkl'.format(LABEL_DICT[str(ARGS.label)]))
            ...
            model=LogisticRegression(C=C, penalty=penalty, solver=solver, l1_ratio=l1_ratio, verbose=0)
            model.fit(X_train, y_train)
