...
## model
arg_parser.add_argument('--model_type', default='SVM', choices=['LR', 'RF', 'SVM', 'DNN', 'DNN_EE', 'GRUD', 'HMM_EF', 'HMM_LF'])
...
model_path = os.path.join(ARGS.working_path, 'model_tuning', 'phase_viii', '48hours', '20220604', 'SVM_423dim_48hrs_Paper(optuna)')
if not os.path.exists(model_path):
    os.makedirs(model_path)
output_path = os.path.join(ARGS.working_path, 'output', 'phase_viii', '48hours', '20220604', 'SVM_423dim_48hrs_Paper(optuna)')
...
# %%
print("[Info]: The SVM hyperparameter tuning of label {0} is processing".format(LABEL_DICT[str(ARGS.label)]))

def objective(trial):
    if ARGS.label == -1 or ARGS.label == 1:
        model = svm.SVC(C=trial.suggest_categorical("C", [1.e-03, 1.e-02, 1.e-01, 1.e+00, 1.e+01, 1.e+02, 1.e+03]),
                        kernel=trial.suggest_categorical("kernel", ["linear", "rbf", "sigmoid"]),
                        gamma=trial.suggest_categorical("gamma", [0.01, 0.001, 0.0001]),
                        probability=True, 
                        random_state=1, 
                        cache_size=5000, 
...
if __name__ == '__main__':
    ...
    if ARGS.trainORvalidation == 'Train':
        ...
        #                         # sampler= optuna.samplers.TPESampler(),
        #                         load_if_exists=True,
        #                         study_name='SVM(NIID and tumor)_20220530112541')
        ...
        C, kernel, gamma = hps['C'], hps['kernel'], hps['gamma']
        ...
        if ARGS.label == -1:
            ...
            # save the model
            model_file = os.path.join(model_path, 'SVM({}).pkl'.format(LABEL_DICT[str(ARGS.label)]))
