...
from sklearn.metrics import f1_score, confusion_matrix, roc_curve, roc_auc_score, auc, classification_report, precision_recall_curve
...
from utils.fusion import Mag, WeightedAverage
...
model_path = os.path.join(ARGS.working_path, 'model_tuning', 'phase_viii', '48hours', '20220604', 'HMMLF_Concat_7dim_48hrs_Paper(optuna)')
if not os.path.exists(model_path):
    os.makedirs(model_path)
output_path = os.path.join(ARGS.working_path, 'output', 'phase_viii', '48hours', '20220604', 'HMMLF_Concat_7dim_48hrs_Paper(optuna)')
...
class RocAucMetricCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        x_val = [self.validation_data[0], self.validation_data[1], self.validation_data[2], self.validation_data[3]]
        y_val = np.argmax(self.validation_data[4], axis=1)
...
def build_model(trial):
    ...
    grud_layer = GRUD(units=trial.suggest_int("units", 24, 156, step=2),
    ...
    dnn_model = Input(shape=(X_train[3].shape[1],))
    input_list.append(dnn_model)

    n_layers = trial.suggest_int("n_layers", 1, 5, step=1)
    for i in range(n_layers):
        dnn_model = Dense(units=trial.suggest_int("n_units_{}".format(i), 32, 512, step=4),
        ...
        if dropout_choice == True:
            dnn_model = Dropout(rate=trial.suggest_float("dropout_val_{}".format(i), 0.0, 0.5, step=0.05))(dnn_model)

    output_model = Concatenate()([dnn_model, grud_model])
    # output_model = Gmu(units=trial.suggest_int("n_units_gmu", 32, 512, step=8))([dnn_model, grud_model])

    output_model = Dense(nclasses_train, activation=None)(output_model)
    ...
    kwargs = {}
    optimizer_selected = trial.suggest_categorical("optimizer", ["RMSprop", "Adam", "Adagrad", "SGD"])
...
class Objective():
    ...
    def __call__(self, trial):
        ...
        model.fit(X_train,
                    ...
                    shuffle=True,
                    batch_size= trial.suggest_categorical("batchsize", [8, 16, 24, 32, 64, 128]),
                    epochs=150,
                    verbose=2,
                    class_weight="auto",
    ...
    def callback(self, study, trial):
        if study.best_trial.number == trial.number:
            self._model.save(os.path.join(model_path, 'HMMLF({0})_trial#{1}.h5'.format(LABEL_DICT[str(ARGS.label)], trial.number))) 
...
if __name__ == '__main__':
    ...
    #                          pruner=optuna.pruners.MedianPruner(),
    #                          load_if_exists=True,
    #                          study_name='HMM_LF(NIID and tumor)_20220606212129')
